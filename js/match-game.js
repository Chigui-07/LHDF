(() => {
  ...
  stage.innerHTML = `<div class="match-game-shell">
    ...
    <main class="physics-match-main">
      ...
      <aside class="physics-panel">
        <p class="eyebrow">PRUEBA 5.2</p>
        <h2 id="turnTitle">Apunta la ficha</h2>
        <p id="turnDescription">Mantén clic, arrastra la flecha y suelta. Las fichas y la pelota reaccionan físicamente.</p>
        <div class="physics-help">
          <div><b>🎯 Apunta</b><span>La flecha marca dirección y potencia.</span></div>
          <div><b>💥 Choques</b><span>Pelota y fichas reciben fuerza al colisionar.</span></div>
          <div><b>🔁 Turnos</b><span>Cuando todo se detiene, responde el rival.</span></div>
        </div>
  ...
  let settleTimer=0, phaseToken=0;
  ...
  function handleGoal(side){if(goalLock)return;goalLock=true;ball.vx=ball.vy=0;eachPiece(p=>{p.vx=p.vy=0;});if(side==='home')homeScore++;else awayScore++;const scorer=player(side,'forward'),club=side==='home'?homeClub:awayClub;addStat(scorer?.id,'goals');goalLog.push({side,scorerId:scorer?.id,minute});$('matchHomeScore').textContent=String(homeScore);$('matchAwayScore').textContent=String(awayScore);$('goalClubLogo').src=logoPath(club.logo);$('goalPlayerName').textContent=scorer?.name||'Delantero';$('goalClubName').textContent=club.name;$('goalScoreText').textContent=`${homeScore} - ${awayScore}`;goalCelebration.hidden=false;setTimeout(()=>{goalCelebration.hidden=true;resetPieces();goalLock=false;resolving=false;if(minute>=MATCH_END){finishMatch();return;}if(side==='home'){playerTurn=false;startAITurn(500);}else beginPlayerTurn();},1800);}

  function showHalftime(next){paused=true;aiming=null;eachPiece(p=>{p.vx=p.vy=0;});ball.vx=ball.vy=0;$('halftimeScore').textContent=`${homeClub.name} ${homeScore} - ${awayScore} ${awayClub.name}`;eventEl.textContent='45’ · Descanso.';halftimeOverlay.classList.add('is-visible');setTimeout(()=>{halftimeOverlay.classList.remove('is-visible');setTimeout(()=>{paused=false;resetPieces();draw();next?.();},320);},1900);}
  function advanceClock(next){const old=minute;minute=Math.min(MATCH_END,minute+3);$('matchClock').textContent=`${String(minute).padStart(2,'0')}'`;if(!halftime&&old<45&&minute>=45){halftime=true;minute=45;$('matchClock').textContent="45'";showHalftime(next);return false;}if(minute>=MATCH_END){finishMatch();return false;}return true;}
  function beginPlayerTurn(){if(!running||paused||minute>=MATCH_END)return;phaseToken++;playerTurn=true;resolving=false;aiming=null;settleTimer=0;turnFooter.textContent='Tu turno';turnTitle.textContent='Apunta una ficha';turnDescription.textContent='Mantén clic, arrastra la flecha y suelta para lanzar la ficha.';movementFill.style.width='0%';selectedPieceLabel.textContent='Selecciona una ficha';draw();}
  function afterAction(side){if(!running||goalLock)return;const continueFlow=()=>{if(side==='home')startAITurn(350);else beginPlayerTurn();};if(advanceClock(continueFlow))continueFlow();}

  function launchPiece(side,role,dx,dy,power){const p=pieces[side][role],len=Math.hypot(dx,dy)||1,nx=dx/len,ny=dy/len,speed=LAUNCH_SPEED[role]*clamp(power,.18,1);p.vx=nx*speed;p.vy=ny*speed;resolving=true;playerTurn=false;settleTimer=0;}
  ...
  function chooseAI(){const options=ROLES.map(role=>({role,d:dist(pieces.away[role],ball)})).sort((a,b)=>a.d-b.d);if(ball.x>760&&dist(pieces.away.goalkeeper,ball)<155)return 'goalkeeper';const forward=options.find(x=>x.role==='forward');if(ball.x<610&&forward?.d<235)return 'forward';return options.find(x=>x.role!=='goalkeeper')?.role||'midfielder';}
  function aiVector(role){const p=pieces.away[role],toBall={x:ball.x-p.x,y:ball.y-p.y},d=Math.hypot(toBall.x,toBall.y)||1;if(d<=170){const goal={x:FIELD.x-40,y:FIELD.y+FIELD.h/2};const gx=goal.x-ball.x,gy=goal.y-ball.y,gl=Math.hypot(gx,gy)||1;const desired={x:ball.x-gx/gl*(p.r+ball.r+7),y:ball.y-gy/gl*(p.r+ball.r+7)};return{x:desired.x-p.x,y:desired.y-p.y,power:clamp(d/110,.55,1)};}return{x:toBall.x,y:toBall.y,power:.9};}
  function startAITurn(delay=0){if(!running||paused||minute>=MATCH_END||goalLock)return;phaseToken++;const token=phaseToken;playerTurn=false;resolving=true;turnFooter.textContent='Turno rival';turnTitle.textContent=`${awayClub.name} ataca`;turnDescription.textContent='El rival busca la pelota y prepara su golpe.';setTimeout(()=>{if(token!==phaseToken||!running||paused||goalLock)return;const role=chooseAI(),v=aiVector(role);eventEl.textContent=`${String(minute).padStart(2,'0')}’ · ${player('away',role).name} va hacia la pelota.`;launchPiece('away',role,v.x,v.y,v.power);},delay);}
  ...
  function physicsLoop(now){const dt=Math.min(.033,Math.max(.001,(now-lastFrame)/1000||.016));lastFrame=now;if(running&&!paused){updatePieces(dt);updateBall(dt);if(resolving&&!goalLock){settleTimer+=dt;if(bodiesSettled()&&settleTimer>.18||settleTimer>3.5){const side=playerTurn?'home':(turnFooter.textContent==='Turno rival'?'away':'home');const wasAway=turnFooter.textContent==='Turno rival';resolving=false;settleTimer=0;afterAction(wasAway?'away':'home');}}draw();}raf=requestAnimationFrame(physicsLoop);}
  ...
  function installLauncher(){const options=document.querySelector('.club-menu-options');if(!options)return;let b=$('clubPlayMatchButton');if(!b){b=document.createElement('button');b.id='clubPlayMatchButton';b.className='club-menu-option club-match-launch is-active';b.type='button';options.appendChild(b);b.addEventListener('click',e=>{e.stopPropagation();window.showInauguralMatch();});}b.innerHTML='<em>Jugable · prueba 5.2</em><strong>Partido inaugural</strong><span>Fichas y pelota con física, IA ofensiva y turnos alternados.</span>';}
